...
export async function createApp() {
    ...
    apiRouter.post("/webhook", express.raw({ type: 'application/json' }), async (req, res) => {
        ...
        let event;
        try {
            event = await StripeService.verifyWebhookSignature(req.body, sig, webhookSecret);
        }
        catch (err) {
        ...
        if (event.type === 'payment_intent.succeeded') {
            ...
            if (db && email) {
                try {
                    ...
                    try {
                        await db.collection('abandoned_carts').doc(email).update({ status: 'converted', convertedAt: admin.firestore.FieldValue.serverTimestamp() });
                    }
                    catch (e) { }
                }
                catch (err) {
    ...
    apiRouter.post("/admin/send-email", async (req, res) => {
        try {
            ...
            await Promise.all(recipients.map((email) => transporter.sendMail({ from: process.env.SMTP_USER, to: email, subject, html })));
    ...
    apiRouter.post("/create-payment-intent", async (req, res) => {
        try {
            ...
            if (db) {
                try {
                    ...
                    for (const item of items) {
                        ...
                        if (plan)
                            total += plan.price * (item.quantity || 1);
                    }
                }
                catch (e) { }
            }
            if (total === 0)
                total = items.reduce((sum, item) => sum + (Number(item.numericPrice) * (item.quantity || 1)), 0);
            ...
            const paymentIntent = await stripe.paymentIntents.create({
                ...
                metadata: { customerName: name || "", customerEmail: email || "", customerPhone: phone || "", taxId: taxId || "", items: JSON.stringify(items.map((i) => i.id)) }
            });
            if (db && email) {
                try {
                    await db.collection('abandoned_carts').doc(email).set({ email, name, items, amount: total, status: 'pending', paymentIntentId: paymentIntent.id }, { merge: true });
                }
                catch (e) { }
            }
    ...
    apiRouter.post("/create-checkout-session", async (req, res) => {
        try {
            ...
            const session = await stripe.checkout.sessions.create({
                payment_method_types: ['card'],
                line_items: items.map((item) => ({ price_data: { currency: 'brl', product_data: { name: item.name }, unit_amount: Math.round(item.numericPrice * 100) }, quantity: item.quantity })),
                mode: 'payment',
        ...
        try {
            ...
            if (session_id) {
                ...
                if (session.payment_status === 'paid')
                    return res.json({ success: true, email: session.customer_details.email, amount: session.amount_total / 100 });
            }
            else if (payment_intent) {
                const pi = await stripe.paymentIntents.retrieve(payment_intent);
                if (pi.status === 'succeeded')
                    return res.json({ success: true, email: pi.receipt_email, amount: pi.amount / 100 });
            }
